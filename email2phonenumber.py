...
from bruteforcers.amazone import Amazone
...
def bruteforce(args, colors, user_agents_instance, proxy_instance, settings):
    ...
    if args.mask:
        possiblePhoneNumbers = phonenumber_supplier.get()
    else:
        possiblePhoneNumbers = phonenumber_supplier.get_from_dump(args.file)

    bruteforcers = get_bruteforcers(args, possiblePhoneNumbers, user_agents_instance, proxy_instance)
...
def get_bruteforcers(args, possible_phone_numbers, user_agents_instance, proxy_instance) -> list:
    ...
    if args.quiet:
        return [twitter]
    return [twitter, Amazone(**bruteforcers_parameters)]
